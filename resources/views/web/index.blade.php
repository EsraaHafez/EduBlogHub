...
<head>
    ...
    .btn-logout i {
        font-size: 18px; /* حجم الأيقونة أصغر */
        margin-right: 8px; /* مساحة بين الأيقونة والنص */
    }
    </style>
</head>
...
<body>
 ...
 -->
            ...
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit" class="btn-logout"><i class="fas fa-sign-out-alt"></i>تسجيل خروج</button>
            </form>
        </nav>
    </header>
    <section class="hero">
    ...
    <div class="posts-grid">
        ...
        <div class="post-card" data-aos="fade-up">
            <!-- إذا كان لديك صورة في قاعدة البيانات، يمكنك إضافتها -->
            <img src="{{ Storage::url($post->image_url) }}" alt="Post Image" class="post-image">
            <div class="post-content">
                <h3 class="post-title">{{ $post->Post_title }}</h3> <!-- عنوان البوست -->
                <p class="post-excerpt">{{ Str::limit($post->Post_content, 100) }}</p> <!-- جزء من المحتوى -->
                <a href="{{ route('displaypost', $post->PostID) }}" class="btn-secondary">Read More</a>
            </div>
        </div>
        @endforeach
    </div>
